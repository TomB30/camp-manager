...
class StaffMembersService {
  ...
  async deleteStaffMember(id: string): Promise<void> {
    ...
    const updatedEvents = events.map(event => ({
      ...event,
      assignedStaffIds: event.assignedStaffIds?.filter((staffId: string) => staffId !== id) || [],
    }));
   ...
   */
  async getStaffMembersByCertification(certificationId: string): Promise<StaffMember[]> {
    const staffMembers = await this.getStaffMembers();
    return staffMembers.filter(s => s.certificationIds?.includes(certificationId));
  }
